...
\cite{yw-website,mcphillips2015yw} aims to provide
...
provided the former \cite{mcphillips2015yw}.
...
language independent.\footnote{\yw\ has been applied to
  Python, \R, and \MATLAB\ scripts already.}  % \cite{mcphillips2015yw}
%
...
they \emph{think} about it. This model is typically not apparent from,
and difficult to tease out of, a script or fine-grained runtime
provenance model that focuses on function calls and program
variables. 
...
%Given a script, YesWorkflow generates a prospective provenance in the form of a workflow characterizing the main building blocks of the script and their data dependencies. 
...
follows.  The script author begins by annotating the computational
blocks and dataflows in the script using expressions of the form
\ywa{@\emph{tag}}\,\textvisiblespace\,\ywa{\emph{value}}. Here,
\ywa{@\emph{tag}} is one of the recognized \yw-keywords, after which
a \ywa{\emph{value}} follows, separated by one or more whitespace
characters.  
...
\yw\ toolkit \cite{mcphillips2015yw,yw-website}. Using simple
...
quickly\footnote{The first author of \cite{bocinsky2014} reported that
  ...
  \cite{mcphillips2015yw} (for an \R\ script used in
