...
\begin{abstract}
  ...
  \emph{without} relying on a runtime provenance recorder. 
  Instead we exploit the common practice of scientists to embed
...
\caption{YW-annotated fragment of a Python script for data
  ...
  templates for raw images (line 5) and corrected images (line 18)
  ...
  8, 11, 21): aliases (lines 4, 16) are used to link such program-level
...
\begin{figure*}[th]
  ...
  \caption{Workflow graph generated from \yw-annotations of the
    ...
    accessing or persisting those data. }
  \label{fig-wfgraph}
\end{figure*}
...
%On the other hand, it is much less well known that an
...
It is, however, important to note the limitations of our approach to reconstructing retrospective
provenance as currently implemented in \YW.  
Because the \YW\ tools are not in any way invoked while a script is executing, 
no dependencies between script inputs, intermediate data products, and final
outputs are directly observed or recorded at run time. Instead, associations between 
input data, parameters, and intermediate and final data products
are inferred based on shared metadata values assigned to
data read or written by the script.  The location
of input and output files, and the portions of file access paths that correspond to
metadata values, are declared using \yw\ annotations.  

For queries $Q_3$, $Q_4$, and $Q_5$ we define and use the \code{depends\_on} relation
that captures the assumption that---for this script---the existence or value of a data product 
depends on another data product if the second item passes through 
a data channel leading to the first, the two data products are assigned at least one shared
metadata variable, and if all of the metadata variables shared by the two data products 
also have shared values.
In future work we plan to add support to \YW\ for explicitly declaring within \yw\ annotations
precisely when such dependencies may correctly be inferred, using the fine-grained 
data dependency rules described in \cite{bowers2012rules}.
